...
Once your PR is merged to ``master``, follow these steps to release new package versions:

#. Manually trigger the ``Release`` GitHub Actions workflow `found here <https://github.com/edx/frontend-enterprise/actions/workflows/release.yml>`_. This workflow runs linting, tests, and builds, then executes ``lerna version`` which increments versions in the appropriate package.json files, creates Git tags, creates GitHub Releases, and opens a PR (``automation/lerna/version`` → ``master``) containing the version-bump commit.
#. Review and merge the auto-generated version-bump PR to ``master``.
#. Manually trigger the ``Publish from package.json`` GitHub Actions workflow `found here <https://github.com/edx/frontend-enterprise/actions/workflows/publish-from-package.yml>`_. This publishes any package versions in the package.json files that are not yet on the NPM registry.
